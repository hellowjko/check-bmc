...
inspur_redfish(){
    ...
    curl -k -w %{http_code} -H "X-Auth-Token: ${token}" -X GET https://${ip}/redfish/v1/Systems/1 > ${mydir}/systems.txt
    sed -i 's/,/\n/g' ${mydir}/systems.txt
    bios_version=$(cat ${mydir}/systems.txt | grep "BiosVersion" | sed 's/"//g' | awk -F": " '{print $2}'|sed 's/\\//g'|sed 's/ /-/g')
    bmc_hostname=$(cat ${mydir}/systems.txt | grep "HostName" | sed 's/"//g' | awk -F: '{print $2}'|sed -e 's/[[:space:]]//g')

    curl -k -w %{http_code} -H "X-Auth-Token: ${token}" -X GET https://${ip}/redfish/v1/Managers/1 > ${mydir}/managers.txt
    sed -i 's/,/\n/g' ${mydir}/managers.txt
    localtime=$(cat ${mydir}/managers.txt | grep "DateTimeLocalOffset" | sed 's/"//g' | awk -F: '{print $2":"$3}'|sed -e 's/[[:space:]]//g')
